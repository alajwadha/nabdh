...
export default function Body() {
  ...
  // BMI gauge on a 15–35 scale
  const bmiPos = Math.max(0, Math.min(100, ((b.value - 15) / 20) * 100));
  ...
  return (
    <Screen>
      ...
      <View style={{ flexDirection: 'row', gap: spacing.md }}>
        <Vital label="REST HR" value={s?.restingHeartRate ?? 58} unit="bpm" color={tiles.peach} />
        <Vital label="HRV" value={s?.hrvSdnn ?? 48} unit="ms" color={tiles.lav} />
        <Vital label="BLOOD O₂" value={s?.spo2 ?? 97} unit="%" color={tiles.blue} />
      </View>
      ...
      <View style={{ backgroundColor: bmiBandColor.bg, borderRadius: radii.xl, padding: spacing.lg }}>
        ...
        <View style={{ height: 14, borderRadius: 99, backgroundColor: colors.navBg, marginTop: 10, position: 'relative', overflow: 'hidden' }}>
          {/* healthy band 18.5–25 → 17.5%–50% on a 15–35 scale */}
          <View style={{ position: 'absolute', left: '17.5%', width: '32.5%', top: 0, bottom: 0, backgroundColor: colors.accent, opacity: 0.4 }} />
          <View style={{ position: 'absolute', left: '50%', width: '25%', top: 0, bottom: 0, backgroundColor: '#E0A24E', opacity: 0.45 }} />
        ...
        {detailed && (
          <AppText variant="caption" color={bmiBandColor.ink} style={{ marginTop: 6, opacity: 0.85 }}>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: color.bg, borderRadius: radii.xl, padding: spacing.md }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: 2, marginTop: 4 }}>
        ...
        <AppText variant="caption" color={color.ink} style={{ marginBottom: 3, fontSize: 10 }}>{unit}</AppText>
      </View>
    </View>
  );
}
